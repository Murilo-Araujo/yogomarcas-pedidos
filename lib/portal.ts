...
export function variantProduct(p:Product,f?:Flavor):Product{const price=f?f.package_price:p.package_price;return {...p,bundle_units:BUNDLE_UNITS,package_price:price,bundle_price:bundlePrice(price),...(f?{package_weight_grams:f.package_weight_grams,available:p.available&&f.available&&f.active}:{})};}
