...
export type CheckoutDetails={name:string;city:string;state:string};
...
function details(value:unknown):CheckoutDetails|null{
 ...
 return {name:v.name,city:v.city,state:v.state};
}
...
export function readCheckoutDetails(profile:CheckoutProfile):CheckoutDetails{
 const saved=details(profile.checkout_details)||{name:profile.contact_name||'',city:'',state:'PR'};
