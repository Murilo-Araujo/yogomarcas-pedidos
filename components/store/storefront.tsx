...
import {EMPTY_DELIVERY_ADDRESS,formatPostalCode,normalizeDeliveryAddress} from '@/lib/delivery-address';
...
const EMPTY_CUSTOMER={name:'',company:'',phone:'',city:'',state:'PR',notes:'',...EMPTY_DELIVERY_ADDRESS};
 ...
 >
  ...
  {!cart.length?<div className="empty-cart order-empty"><span className="order-empty-icon"><ShoppingBag size={38} strokeWidth={1.5} aria-hidden="true"/></span><h3>Seu próximo pedido começa aqui</h3><p>Explore as linhas e escolha os sabores para sua loja.</p><button type="button" className="btn primary" onClick={()=>setCartOpen(false)}>Escolher produtos<ArrowRight size={18}/></button></div>:<>
   ...
   {checkout&&<form id={ORDER_CHECKOUT_FORM_ID} onSubmit={send} className="checkout-form order-checkout-form">
    <div className="order-checkout-heading"><h2 tabIndex={-1} data-order-step-heading>Contato e entrega</h2><p>Seu nome e endereço ficam salvos para os próximos pedidos. Você pode editar quando precisar.</p></div>
    ...
    <fieldset className="order-contact-fields" disabled={sending}>
     <legend className="sr-only">Responsável pelo pedido</legend>
     <Field label="Seu nome" required value={customer.name} onChange={value=>updateCheckout('name',value)} autoComplete="name"/>
    </fieldset>
    <fieldset className="order-contact-fields order-address-fields" disabled={sending}>
     <legend>Endereço de entrega</legend>
     <div className="delivery-address-grid">
      <Field label="CEP" required value={customer.postal_code} onChange={value=>updateCheckout('postal_code',formatPostalCode(value))} autoComplete="shipping postal-code" inputMode="numeric" pattern="[0-9]{5}-[0-9]{3}" maxLength={9} placeholder="00000-000"/>
      <Field label="Rua / avenida" className="address-full" required value={customer.street} onChange={value=>updateCheckout('street',value)} autoComplete="shipping address-line1"/>
      <Field label="Número" required minLength={1} maxLength={20} value={customer.number} onChange={value=>updateCheckout('number',value)} placeholder="Número ou S/N"/>
      <Field label="Bairro" required value={customer.neighborhood} onChange={value=>updateCheckout('neighborhood',value)} autoComplete="shipping address-level3"/>
      <Field label="Complemento (opcional)" className="address-full" value={customer.complement} onChange={value=>updateCheckout('complement',value)} autoComplete="shipping address-line2" placeholder="Sala, bloco, apartamento..."/>
      <div className="address-city-state address-full"><Field label="Cidade" required value={customer.city} onChange={value=>updateCheckout('city',value)} autoComplete="shipping address-level2"/><label className="field"><span>Estado *</span><Choice value={customer.state} onChange={value=>updateCheckout('state',value)} options={STATES.map(state=>({label:state,value:state}))} label="Estado" disabled={sending}/></label></div>
      <Field label="Ponto de referência (opcional)" className="address-full" value={customer.reference} onChange={value=>updateCheckout('reference',value)} placeholder="Algo que ajude a encontrar o local"/>
     </div>
     <p className="delivery-address-hint">Este endereço será incluído na mensagem do pedido.</p>
    </fieldset>
    <Disclosure title="Adicionar observação"><label className="field"><span>Observações do pedido</span><textarea rows={3} maxLength={1000} disabled={sending} value={customer.notes} onChange={event=>setCustomer({...customer,notes:event.target.value})} placeholder="O que precisamos saber sobre este pedido?"/></label></Disclosure>
...
function Field({label,value,onChange,required=false,type='text',autoComplete,readOnly=false,className='',inputMode,pattern,placeholder,maxLength=200,minLength}:{label:string;value:string;onChange:(v:string)=>void;required?:boolean;type?:string;autoComplete?:string;readOnly?:boolean;className?:string;inputMode?:React.HTMLAttributes<HTMLInputElement>['inputMode'];pattern?:string;placeholder?:string;maxLength?:number;minLength?:number}){return <label className={`field ${className}`}><span>{label}{required?' *':''}</span><input readOnly={readOnly} required={required} type={type} value={value} onChange={e=>onChange(e.target.value)} autoComplete={autoComplete} inputMode={inputMode} pattern={pattern} placeholder={placeholder} maxLength={maxLength} minLength={minLength??(required&&type==='text'?2:undefined)}/></label>;}
